\documentclass[a4paper,10pt]{article}
\usepackage[utf8]{inputenc}
\usepackage[brazilian]{babel}
\usepackage[justification=centering]{caption}
\usepackage[hmargin=2cm,vmargin=3.5cm,bmargin=2cm]{geometry}
\usepackage[table,xcdraw]{xcolor}
\usepackage{indentfirst}
\usepackage{amsmath}
\usepackage{graphicx}
\usepackage{makeidx}
\usepackage{tocbibind}
 
 \begin{document}	
	\begin{titlepage}
		\begin{center}
		{\large UNIVERSIDADE ESTADUAL DE MARINGÁ}\\[0.2cm]
		{\large CENTRO DE CIÊNCIAS EXATAS}\\[0.2cm]
		{\large DEPARTAMENTO DE FÍSICA}\\[0.2cm]
		{\large LABORATÓRIO DE FÍSICA MODERNA}\\[7.0cm]
		{\bf \huge Gotas de Millikan}\\[7.0cm]
		\end{center}
	{\large Adão Murillo dos Santos \hfill RA:100126}\\[0.7cm]
	{\large João Marcos Fávaro Lopes \hfill RA:98327}\\[0.7cm]
	{\large Lucas Maquedano da Silva \hfill RA:98901}\\[0.7cm]
	{\large Pedro Haerter Pinto \hfill RA:100852}\\[0.7cm]
	{\large TURMA:32 \hfill Professor:Nelson Guilherme Castelli
	Astrath}
	
	\vfill
		\begin{center}
		{\large Maringá,2018}
		\end{center}
	\end{titlepage}

	\tableofcontents
	\include{Res}
	\include{FundTeo}
	\include{DesExp}	
	\include{Concl}
\vspace{3cm}
	\begin{thebibliography}{1}
    	\bibitem{PASCO}PASCO, \it{MILLIKAN OIL DROP APPARATUS}, Instruction Manual and Experiment Guide for the PASCO scientific Model AP-8210.
		\bibitem{Evangelista}Luiz Roberto Evangelista,\it{PERSPECTIVA EM HISTORIA DA FISICA},Maringá:Ciência Moderna, 2011.
	\end{thebibliography}	
	
\end{document}

